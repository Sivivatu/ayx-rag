...
class SitemapDownloader:
    ...
    def is_retryable_error(self, error: Exception) -> bool:
        ...
        Note:
            ...
            Non-retryable: 4xx errors, other exceptions
        """
    ...
    def _attempt_download(
        ...
        Returns:
            Tuple of (downloaded_file_path, file_size)
    ...
    def download(self, progress_callback: ProgressCallback | None = None) -> DownloadResult:
        ...
        try:
                ...
                try:
                    ...
                    if attempt < self.config.max_retries and self.is_retryable_error(e):
                        ...
                        logger.warning(
                            ...
                            f"Retrying in {delay:.1f}s (retry {retry_count}/{self.config.max_retries})"
                        )
                        time.sleep(delay)
                    else:
            ...
            error_msg = f"Timeout during download{retry_suffix}: {e}"
            logger.error(error_msg)
            return DownloadResult.failure_result(error_msg, duration, retry_count=retry_count)

        except httpx.HTTPError as e:
            duration = time.time() - start_time
            retry_suffix = f" after {retry_count} retries" if retry_count > 0 else ""
            error_msg = f"HTTP error during download{retry_suffix}: {e}"
            logger.error(error_msg)
            return DownloadResult.failure_result(error_msg, duration, retry_count=retry_count)
            ...
            duration = time.time() - start_time
            retry_suffix = f" after {retry_count} retries" if retry_count > 0 else ""
            error_msg = f"Unexpected error during download{retry_suffix}: {e}"
            logger.error(error_msg)
            return DownloadResult.failure_result(error_msg, duration, retry_count=retry_count)
