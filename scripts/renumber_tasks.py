...
def apply_fixes(lines: List[str], tasks: List[Task]) -> List[str]:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if i == 75 and 'T020' in line and 'integration test' in line and 'US1' in line:
            new_lines.append('- [ ] T022 [P] [US1] Write unit test test_check_disk_space_sufficient (optional) in packages/sitemap-download/tests/unit/test_downloader.py\n')
            new_lines.append('- [ ] T023 [P] [US1] Write unit test test_check_disk_space_insufficient (optional) in packages/sitemap-download/tests/unit/test_downloader.py\n')
        
        # Fix 5: Add disk space check implementation after T022 (__init__)
        if i == 80 and 'T022' in line and '__init__' in line:
            new_lines.append('- [ ] DISK_CHECK [US1] Implement check_disk_space() pre-flight check (optional) in packages/sitemap-download/src/sitemap_download/downloader.py\n')
        
        # Fix 6: Add FR-009 implementation after T027 (timeout config)  
        if i == 85 and 'T027' in line and 'timeout configuration' in line:
            new_lines.append('- [ ] FR009 [US1] Implement custom HTTP headers (User-Agent, Accept-Encoding) per FR-009 in packages/sitemap-download/src/sitemap_download/downloader.py\n')
