...
'''
    ...
    for line in lines:
        ...
        if in_us1_impl and line.startswith('- [ ] T'):
            ...
            if 'T025 [US1]' in line and '__init__' in line and not disk_check_added:
                result_lines.append('- [ ] T026 [US1] Implement check_disk_space() pre-flight check (optional) in packages/sitemap-download/src/sitemap_download/downloader.py')
                disk_check_added = True
            
            # Add FR-009 after T031 (timeout)
            elif 'T031 [US1]' in line and 'timeout' in line and not fr009_added:
                result_lines.append('- [ ] T032 [US1] Implement custom HTTP headers (User-Agent, Accept-Encoding) per FR-009 in packages/sitemap-download/src/sitemap_download/downloader.py')
                fr009_added = True
    ...
    temp_lines = []
    for line in lines:
        if line.startswith('- [ ] T') and not '[US1]' in line:  # Skip Phase 3 US1 tasks
    ...
    content = content.replace('T013-T020 can all run in parallel', 'T014-T021 can all run in parallel')
